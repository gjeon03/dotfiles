...
4. `.claude/rules/` 디렉토리에 모듈화된 규칙 파일을 **항상** 생성한다.
   - `CLAUDE.md`에는 요약만, 상세 규칙은 rules로 분리
   - 프로젝트 규모에 따라 파일 수가 달라질 수 있지만 최소 1개는 생성
   - 예시:
     - `conventions.md` — 코드 컨벤션, 네이밍 규칙
     - `api.md` — API 설계 규칙, 엔드포인트 패턴
