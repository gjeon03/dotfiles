...
prefix 키가 `Ctrl+b`(기본)에서 `Ctrl+a`로 변경되어 있다. 모든 tmux 단축키는 prefix를 먼저 누른 뒤 입력한다.

### 키바인딩

| 키 | 동작 | 기본값 |
|----|------|--------|
| `Ctrl+a` | prefix | `Ctrl+b` |
| `!` | 세로 분할 (좌/우) | `%` |
| `@` | 가로 분할 (상/하) | `"` |
| `h` / `j` / `k` / `l` | pane 크기 조절 (좌/하/상/우, 5칸) | 방향키 |
| `m` | pane 최대화/복원 토글 (반복 가능) | — |
| `r` | 설정 파일 리로드 | — |

> pane 간 이동은 `vim-tmux-navigator` 플러그인이 담당: `Ctrl+h/j/k/l`로 vim 창과 tmux pane을 구분 없이 이동한다.

### 복사 모드 (vi mode)

`prefix + [` 로 복사 모드에 진입하면 vim 키로 스크롤/선택이 가능하다.
...
| 설정 | 값 | 설명 |
|------|-----|------|
| 마우스 | 활성화 | 클릭으로 pane 선택, 드래그로 크기 조절, 스크롤 |
| `escape-time` | `10ms` | Neovim ESC 입력 지연 해소 |
| 터미널 | `tmux-256color` + RGB | truecolor 지원 |

### 플러그인 (TPM)

| 플러그인 | 설명 |
|----------|------|
| `vim-tmux-navigator` | `Ctrl+h/j/k/l`로 vim ↔ tmux pane 이동 |
| `tmux-resurrect` | 세션 저장/복원 (pane 레이아웃, 실행 중 프로세스) |
| `tmux-continuum` | resurrect 자동화 (주기적 저장, tmux 시작 시 자동 복원) |
| `tmux-tokyo-night` | Tokyo Night 테마 |

> TPM 플러그인 설치: tmux 안에서 `prefix + I` (대문자). 업데이트: `prefix + U`.
